SMODS.Sticker {
  ...
  calculate = function(self, card, context)
    if (((context.end_of_round and not context.repetition and not context.individual)
      and card.config.center_key ~= 'j_gift'
      and card.config.center_key ~= 'j_gros_michel'
      and card.config.center_key ~= 'j_cavendish'
    )
      ...
      or (card.config.center_key == 'j_cavendish' and context.cavendish_safe and context.cavendish_card == card)
    )
    then
